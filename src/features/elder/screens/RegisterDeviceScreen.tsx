...
import { createDevice } from "@/features/elder/api/createDevice";
import LoadingIcon from "@/shared/components/icons/loading";
import { deviceIdSchema } from "@/shared/utils/deviceIdSchema";
...
export default function RegisterDeviceScreen() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["myDeviceElder", user.id] });
    ...
    >
      <View style={styles.container}>
        <View style={{ bottom: 60 }}>
          <Title>Não há dispositivo registrado</Title>
        </View>
        <View style={{ height: 20 }}></View>
        <Text style={styles.text}>
          Por favor, digite o ID provido pelo seu dispositivo para registrá-lo
        </Text>
        <View style={{ height: 20 }}></View>
        <TextInput
          style={styles.input}
        ...
        />
        {error && (
          <Text style={styles.error}>O ID do dispositivo deve ser válido</Text>
        )}
